...
################ If this is the first time to run, use this class to download the movielens
...
filename=cwd+'/Dataloader/data/movielens/ml-1m/Fixed_overall_mat.pkl'
...
cfg=ReadYAML(cwd+'/Config/Config_SGHMC_PNP_movielens1m.yaml')
...
for runs in range(num_runs):
    ...
    # Get pretrain data
    Active_BALD._get_pretrain_data(target_dim=None,pretrain_number=cfg['Pretrain_Settings']['pretrain_number'])
    ...
    Active_RAND_PNP.train_BNN(counter_loop=0, max_selection=max_selection, flag_pretrain=True,
                              ...
                              target_dim=None,valid_data=valid_data_input_tensor,flag_imputation=flag_imputation,
    ...
    W_sample_RAND = Active_RAND.train_BNN(pretrain_data, eps=args.step_sghmc, max_sample_size=40, tot_epoch=tot_epoch+0,
                                          ...
                                          target_dim=None,flag_reset_optim=flag_reset_optim,flag_imputation=flag_imputation,valid_data=None,valid_data_target=None,sigma_out=sigma_out,scale_data=args.scale_data,noisy_update=args.noisy_update)
    ...
    while counter_selection<max_selection:
        ...
        observed_train_RAND, train_pool_data_RAND, flag_full, num_selected = Active_RAND.base_random_select_training(
            ...
            sigma_out=sigma_out, balance_prop=balance_coef, flag_initial=flag_init_train,Select_Split=Select_Split,selection_scheme=selection_scheme
            )
